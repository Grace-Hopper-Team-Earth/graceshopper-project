...
//PUT route to update quantity of item in cart
router.put("/:teaId/:credential", async (req, res, next) => {
  try {
    let user = await User.findByToken(req.params.credential);
    let cart = await Cart.findOne({
    ...
    const teaToEdit = await Tea.findOne({
      where: {
        cartId: cart.id,
        teaId: req.params.teaId,
      },
...
router.delete('/:cartId/:teaId', async (req, res, next)=> {
  //search for the record in join table where cartId and teaId match
  try {
    const cartTeaRecord = await CartTea.findOne({
      where: {
        cartId: req.params.cartId,
        teaId: req.params.teaId
      }
    })
    await cartTeaRecord.destroy()
    res.sendStatus(200)
  } catch (error) {
    next (error)
  }
